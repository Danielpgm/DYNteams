...
function getBook(data) {
    ...
    data.forEach((element) => {
        if((element.no) < 7){
            output += `
                    <div class="card mx-1 my-3 cardRecomm">
                        <a data-bs-toggle="modal" data-bs-target="#viewDetail" onclick="detailBook('${element.no-1}')">
                            <img class="img-fluid coverBook" alt="100%x280" src="assets/book/${element.id_book}.jpg">
                            <div class="card-body">
        ...
        popular += `
                <div class="card cardPopular">
                    <a data-bs-toggle="modal" data-bs-target="#viewDetail" onclick="detailBook('${element.no-1}')">
                        <img class="img-thumbnail coverBookPopular" alt="100%x280" src="assets/book/${element.id_book}.jpg">
                        <div class="card-body">
                            <h6 class="card-title fw-bold text-center"><b>${element.title}</b></h6>
                            <p class="author text-center">${element.author} <br>(${element.year})</p>
                        </div>
